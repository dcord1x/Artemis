...
After parse, `/check-duplicates` runs in the background and badges any flagged cards (red = exact match, amber = possible match). When the analyst clicks Save, if any selected incidents are flagged the **DupReviewModal** opens. Each flagged item shows incoming vs matched narrative previews; possible duplicates can be toggled Skip/Import anyway; exact duplicates are always excluded. Only analyst-approved items are sent to `/bulk-save`.
...
└── frontend/
    ...
    │   │   ├── GisMapModal.tsx   ← Geocoding map modal
    │   │   └── DupReviewModal.tsx← Pre-save duplicate review modal (Skip/Import)
    │   │
    │   └── pages/
