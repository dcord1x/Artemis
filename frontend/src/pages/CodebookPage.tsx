...
const CODEBOOK_ENTRIES: CodebookEntry[] = [
  ...
  // ── Initial Contact / Approach — stage-detail fields
  {
    field: 'approach_method', label: 'Approach Method',
    definition: 'How the client first made contact with the worker. Coded as additional detail for the Initial Contact stage.',
    tab: 'Incident-Level Coding', level: 'case', type: 'select',
    allowedValues: 'street_approach · online_digital · referral · venue_based · phone_text · vehicle_based · third_party_arranged · unknown_unclear · other',
    codingRule: 'Code the method described in the narrative. street_approach = client approached on foot in a street setting. online_digital = initial contact via app, website, or messaging platform. vehicle_based = client initiated contact from within a vehicle. unknown_unclear = the narrative does not describe how first contact was made.',
    unclearRule: 'Do not infer the approach method if the narrative does not describe how first contact was made. Code unknown_unclear when ambiguous.',
    methodologicalBasis: 'Approach fields provide additional detail for the Initial Contact stage. They support stage reconstruction by recording how contact began when the narrative provides this information. They should not be interpreted as evidence of offender motive.',
  },
  {
    field: 'approach_setting', label: 'Approach Setting',
    definition: 'The physical or digital setting in which first contact occurred. Coded as additional detail for the Initial Contact stage.',
    tab: 'Incident-Level Coding', level: 'case', type: 'select',
    allowedValues: 'public_street · online_platform · venue_indoor · private_space · vehicle · unknown · other',
    codingRule: 'Code the setting described in the narrative at the time of first contact. public_street = open street or public outdoor space. vehicle = the first point of contact was inside or immediately adjacent to a vehicle. unknown = not described.',
    unclearRule: 'Code the setting at the moment of first contact, not the setting where harm occurred.',
    methodologicalBasis: 'Approach fields provide additional detail for the Initial Contact stage. They support stage reconstruction by recording how contact began when the narrative provides this information. They should not be interpreted as evidence of offender motive.',
  },
  {
    field: 'approach_mobility_context', label: 'Approach Mobility Context',
    definition: 'Whether the worker or client was stationary or in motion at the point of first contact. Coded as additional detail for the Initial Contact stage.',
    tab: 'Incident-Level Coding', level: 'case', type: 'select',
    allowedValues: 'stationary · mobile_on_foot · mobile_in_vehicle · transitioning · unknown',
    codingRule: 'Code the mobility state at the point of first contact. stationary = neither party was moving at the moment of contact. mobile_in_vehicle = one or both parties were in a moving vehicle. transitioning = one party was entering or exiting a vehicle at the point of contact.',
    unclearRule: 'If the narrative describes movement before contact but not at the moment of contact, code unknown.',
    methodologicalBasis: 'Approach fields provide additional detail for the Initial Contact stage. They support stage reconstruction by recording how contact began when the narrative provides this information. They should not be interpreted as evidence of offender motive.',
  },
  {
    field: 'client_known_at_contact', label: 'Client Known at Contact',
    definition: 'Whether the client was previously known to the worker at the time of initial contact.',
    tab: 'Incident-Level Coding', level: 'case', type: 'select',
    allowedValues: 'yes_known · first_contact · unclear',
    codingRule: 'yes_known = the narrative explicitly states the client was a known or repeat client. first_contact = the narrative explicitly states or implies this was the first contact. unclear = the narrative does not provide enough information to determine prior contact history.',
    unclearRule: 'Do not infer prior contact from the client type or approach method alone. Code unclear if the narrative does not address prior contact history.',
    methodologicalBasis: 'Approach fields provide additional detail for the Initial Contact stage. They support stage reconstruction by recording how contact began when the narrative provides this information. They should not be interpreted as evidence of offender motive.',
  },
  {
    field: 'initial_contact_visibility', label: 'Initial Contact Visibility',
    definition: 'How observable the initial contact was to potential bystanders at the time it occurred. This is a stage-detail field for the Initial Contact stage.',
    tab: 'Incident-Level Coding', level: 'case', type: 'select',
    allowedValues: 'visible · limited_visibility · not_visible · unclear · not_stated',
    codingRule: 'Code the situational conditions at the moment of first contact. visible = contact occurred where bystanders could plausibly observe it. not_visible = contact occurred in a location structurally shielded from observation. limited_visibility = partial obstruction or low-light conditions. not_stated = the narrative does not describe the visibility conditions.',
    unclearRule: 'Visibility is a property of the setting and situational conditions, not a record of whether anyone actually witnessed the contact. Do not code visible simply because the setting was a public street if the encounter was not observable to bystanders.',
    methodologicalBasis: 'Approach fields provide additional detail for the Initial Contact stage. They support stage reconstruction by recording how contact began when the narrative provides this information. They should not be interpreted as evidence of offender motive.',
  },
  {
    field: 'initial_contact_guardianship', label: 'Initial Contact Guardianship',
    definition: 'Whether capable guardianship was present at the point of first contact. This is a stage-detail field for the Initial Contact stage.',
    tab: 'Incident-Level Coding', level: 'case', type: 'select',
    allowedValues: 'present · limited_reduced · absent · unclear · not_stated',
    codingRule: 'Code whether persons capable of intervening or deterring harm were present at the moment of first contact. present = other persons nearby who could plausibly intervene. limited_reduced = some persons nearby but limited capacity or likelihood of intervention. absent = no persons nearby capable of intervention.',
    unclearRule: 'Guardianship is not equivalent to the presence of any other person — the capacity to intervene matters. Do not code present simply because the contact occurred in a public area.',
    methodologicalBasis: 'Approach fields provide additional detail for the Initial Contact stage. They support stage reconstruction by recording how contact began when the narrative provides this information. They should not be interpreted as evidence of offender motive.',
  },
];
